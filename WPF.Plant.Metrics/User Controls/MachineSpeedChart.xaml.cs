...
{
    /// <summary>
    /// Interaction logic for MachineSpeedChart.xaml
    /// </summary>
    public partial class MachineSpeedChart : UserControl, INotifyPropertyChanged
    {
        private double _axisMax;
        private double _axisMin;
        private double _trend;
        private double _currentLecture;
        public MachineSpeedChart()
        {
            InitializeComponent();

            var mapper = Mappers.Xy<MachineSpeedModel>()
                .X(model => model.DateTime.Ticks)   //use DateTime.Ticks as X
                .Y(model => model.Value);           //use the value property as Y

            //lets save the mapper globally.
            Charting.For<MachineSpeedModel>(mapper);

            //the values property will store our values array
            ChartValues = new GearedValues<MachineSpeedModel>();
        ...
        public GearedValues<MachineSpeedModel> ChartValues { get; set; }
        ...
        private void Read()
        {
            ...
            while (IsReading)
            {
                ...
                ChartValues.Add(new MachineSpeedModel
                {
                    DateTime = now,
                    Value = _trend
                });
